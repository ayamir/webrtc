...
int32_t Recorder::Start(const std::string& path) {
    ...
    av_log_set_callback([](void* ptr, int level, const char* fmt, va_list vl) {
        ...
        RTC_LOG(LS_INFO) << "Log for ffmpeg: " << buf;
    });
    av_log_set_level(AV_LOG_INFO);
    auto dot_pos = path.find_last_of(".");
    if (dot_pos == std::string::npos) {
        RTC_LOG(LS_ERROR) << "Recorder::Start error, invalid path";
        return -9;
    }
    std::string ext = path.substr(dot_pos + 1);
    const char* filename = path.c_str();
    auto out_format = av_guess_format("matroska", NULL, NULL);
    if (!out_format) {
        RTC_LOG(LS_ERROR) << "Recorder::Start error, guess format failed for "
                          << ext;
    }
    avformat_alloc_output_context2(&context_, out_format, NULL, filename);
    if (!context_) {
